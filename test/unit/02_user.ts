...
import { BASE_URL , Global} from '../helper/config';
...
describe('User', () => {
    ...
    step('Register with valid data', (done) => {
        Global.USER.EMAIL = internet.email();
        Global.USER.PASSWORD = getSHA512Hash(internet.password(8));
        const data = {
    ...
    step('Login user with invalid password', (done) => {
        const data = {
            email: Global.USER.EMAIL,
            password: getSHA512Hash(internet.password(8))
        };
        chai.request(BASE_URL)
            .post('/users/login')
            .send(data)
